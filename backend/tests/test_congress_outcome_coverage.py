...
from app.models import AppSetting, Event, Filing, GovernmentContractAction, Member, Security, TradeOutcome, Transaction
...
def test_events_endpoint_uses_persisted_congress_outcome_fields(monkeypatch):
    ...
    try:
        ...
        assert page.items[0].pnl_pct == 10.0
        assert page.items[0].return_pct == 10.0
    finally:
        db.close()
...
def test_recent_ingest_runs_outcome_coverage_for_new_events(monkeypatch):
    ...
    result = recent_module.run_recent_congress_ingest(days=30, pages=1, limit=1, sleep_s=0)
