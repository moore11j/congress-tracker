...
def test_crontab_schedules_bounded_daily_digest_and_intraday_jobs():
    ...
    assert "5 2 * * * cd /app && python -m app.jobs.refresh_seo_snapshots --entity-type ticker --batch --limit 250" in crontab
    ...
    assert "55 2 * * * cd /app && python -m app.jobs.refresh_seo_snapshots --entity-type ticker --batch --include-existing --limit 250" in crontab
