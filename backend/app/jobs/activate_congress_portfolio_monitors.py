"""Activate daily prospective monitoring for supported individual Congress portfolios.

This deliberately does not activate broader Congress, insider, or cross-source
backtests.  Each needs its own point-in-time candidate resolver before it can
produce subscriber-facing model events.
"""
...
def activate(*, apply: bool) -> dict:
    ...
    with SessionLocal() as db:
        ...
        candidates: list[tuple[StrategyDefinition, dict]] = []
        ...
        for strategy, rule in candidates:
            existing = db.execute(
            ...
            if not apply:
                result["activated"].append({"slug": strategy.slug, "memberBioguideId": rule["member_bioguide_id"], "mode": "dry_run"})
                continue
            next_version = int(
            ...
            version = StrategyVersion(
                ...
                universe_json=json.dumps({"label": "Congress Trades", "source": "congress_trade_disclosures", "memberBioguideId": rule["member_bioguide_id"]}, sort_keys=True, separators=(",", ":")),
                methodology="Daily prospective monitoring of newly ingested public congressional disclosures. Purchases enter on the next available trading day; matching reported sales exit the modeled issuer position.",
                effective_from=today,
                created_by="activate_congress_portfolio_monitors",
            )
            db.add(version)
            result["activated"].append({"slug": strategy.slug, "memberBioguideId": rule["member_bioguide_id"], "version": next_version})
        if apply:
            db.commit()
        return result
