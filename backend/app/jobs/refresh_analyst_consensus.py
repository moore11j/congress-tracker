    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        run = start_ingestion_run(
            db,
            "analyst_consensus_daily_refresh",
        ...
        for symbol in planned:
            attempted += 1
            result = ingest_symbol_consensus(db, symbol, observed_at=observed_at)
            results.append(result)
            if result.get("status") in {"unsupported", "provider_error"}:
                failed += 1
                provider_errors.append({"symbol": symbol, "error": result.get("error") or result.get("provider_error")})
            else:
                succeeded += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Refresh stored Analyst Consensus snapshots from FMP stable endpoints.")
    parser.add_argument("--symbols", help="Optional comma-separated symbols. Defaults to the eligible Walnut equity universe.")
    parser.add_argument("--limit", type=int, default=None, help="Maximum symbols to process.")
