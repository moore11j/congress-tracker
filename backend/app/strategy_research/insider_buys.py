...
from app.models import InsiderTransactionNormalized
...
def run_research(db: Session, config: ResearchConfig, *, role: InsiderRole) -> dict[str, object]:
    ...
    role_counts = _role_counts(db, universe=config.universe, start_date=signal_start, end_date=config.end_date)
    return {
        "metadata": {
            ...
            "signal_source": "insider_transactions_normalized open_market_purchase rows",
...
def main() -> None:
    ...
    with SessionLocal() as db:
        result = run_research(db, config, role=role)
    if args.json:
