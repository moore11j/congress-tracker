...
from app.db import (
    ...
    ensure_fundamentals_snapshot_schema,
...
def _run_profile_overview_prewarm(*, force_refresh: bool = False) -> dict[str, object]:
    """Warm the shared Profiles dashboard payloads at startup or on a schedule.

    A freshly deployed process can otherwise make the first visitor wait for
    aggregate queries. Scheduled callers force a rebuild so the persistent
    cache is renewed rather than merely reusing its stale-while-revalidate
    fallback.
    """
    activity_per_type = 5
    cache_keys = (
    ...
    return {
        "status": "ok",
        "force_refresh": force_refresh,
        "cache_entries_refreshed": len(refreshed_keys),
        "cache_keys": [str(key[0]) for key in refreshed_keys],
    }
