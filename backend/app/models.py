...
class StrategyVersion(Base):
    """An immutable, publishable methodology revision for a strategy."""

    __tablename__ = "strategy_versions"
    __table_args__ = (
        UniqueConstraint("strategy_id", "version", name="uq_strategy_versions_strategy_version"),
        Index("ix_strategy_versions_strategy_effective", "strategy_id", "effective_from", "effective_to"),
    )
    ...
    status: Mapped[str] = mapped_column(Text, default="draft", server_default="draft", nullable=False)
    rules_json: Mapped[str] = mapped_column(Text, default="{}", server_default="{}", nullable=False)
    parameters_json: Mapped[str] = mapped_column(Text, default="{}", server_default="{}", nullable=False)
    universe_json: Mapped[str] = mapped_column(Text, default="{}", server_default="{}", nullable=False)
    methodology: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class StrategyEvaluationRun(Base):
    """One prospective evaluation/rebalance attempt. Historical research stays in StrategyBacktestRun."""

    __tablename__ = "strategy_evaluation_runs"
    __table_args__ = (
        UniqueConstraint("strategy_id", "idempotency_key", name="uq_strategy_evaluation_runs_idempotency"),
        Index("ix_strategy_evaluation_runs_strategy_evaluated", "strategy_id", "evaluation_date"),
        Index("ix_strategy_evaluation_runs_status", "status", "scheduled_for"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    strategy_id: Mapped[int] = mapped_column(nullable=False)
    strategy_version_id: Mapped[int] = mapped_column(nullable=False)
    idempotency_key: Mapped[str] = mapped_column(Text, nullable=False)
    evaluation_date: Mapped[date] = mapped_column(nullable=False)
    scheduled_for: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    executed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    status: Mapped[str] = mapped_column(Text, default="pending", server_default="pending", nullable=False)
    universe_count: Mapped[int] = mapped_column(default=0, server_default=text("0"), nullable=False)
    qualifying_count: Mapped[int] = mapped_column(default=0, server_default=text("0"), nullable=False)
    metadata_json: Mapped[str] = mapped_column(Text, default="{}", server_default="{}", nullable=False)
    error: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
...
class StrategyLiveHolding(Base):
    """Current prospective model holdings, kept separate from historical research snapshots."""

    __tablename__ = "strategy_live_holdings"
    __table_args__ = (
        UniqueConstraint("strategy_id", "symbol", name="uq_strategy_live_holdings_strategy_symbol"),
        Index("ix_strategy_live_holdings_strategy_rank", "strategy_id", "rank"),
        Index("ix_strategy_live_holdings_strategy_as_of", "strategy_id", "as_of_date"),
        Index("ix_strategy_live_holdings_symbol", "symbol"),
    )
    ...
    opening_trade_id: Mapped[int] = mapped_column(nullable=False)
    security_id: Mapped[Optional[int]] = mapped_column(nullable=True)
    symbol: Mapped[str] = mapped_column(Text, nullable=False)
    ticker_at_time: Mapped[str] = mapped_column(Text, nullable=False)
    company_name: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    qualification_snapshot_json: Mapped[str] = mapped_column(Text, default="{}", server_default="{}", nullable=False)
    as_of_date: Mapped[date] = mapped_column(nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
...
class StrategyTrade(Base):
    """Auditable model-portfolio trade or rebalance change generated by a live evaluation."""

    __tablename__ = "strategy_trades"
    __table_args__ = (
        UniqueConstraint("strategy_run_id", "symbol", "action", name="uq_strategy_trades_run_symbol_action"),
        Index("ix_strategy_trades_strategy_effective", "strategy_id", "effective_date"),
        Index("ix_strategy_trades_strategy_status", "strategy_id", "status"),
        Index("ix_strategy_trades_symbol_effective", "symbol", "effective_date"),
    )
    ...
    ticker_at_time: Mapped[str] = mapped_column(Text, nullable=False)
    action: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(Text, default="open", server_default="open", nullable=False)
    signal_date: Mapped[Optional[date]] = mapped_column(nullable=True)
    ...
    score_at_exit: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    source_count_at_entry: Mapped[Optional[int]] = mapped_column(nullable=True)
    qualification_snapshot_json: Mapped[str] = mapped_column(Text, default="{}", server_default="{}", nullable=False)
...
class StrategyEvent(Base):
    ...
    __table_args__ = (
        UniqueConstraint("dedupe_key", name="uq_strategy_events_dedupe_key"),
        Index("ix_strategy_events_strategy_occurred", "strategy_id", "occurred_at"),
        Index("ix_strategy_events_type_occurred", "event_type", "occurred_at"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    strategy_id: Mapped[int] = mapped_column(nullable=False)
    strategy_version_id: Mapped[int] = mapped_column(nullable=False)
    strategy_run_id: Mapped[Optional[int]] = mapped_column(nullable=True)
    strategy_trade_id: Mapped[Optional[int]] = mapped_column(nullable=True)
    security_id: Mapped[Optional[int]] = mapped_column(nullable=True)
    symbol: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class StrategySubscription(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "strategy_id", name="uq_strategy_subscriptions_user_strategy"),
        Index("ix_strategy_subscriptions_strategy_active", "strategy_id", "is_active"),
        Index("ix_strategy_subscriptions_user_active", "user_id", "is_active"),
    )
...
class StrategyEventDelivery(Base):
    __tablename__ = "strategy_event_deliveries"
    __table_args__ = (
        UniqueConstraint("idempotency_key", name="uq_strategy_event_deliveries_idempotency"),
        Index("ix_strategy_event_deliveries_event_status", "strategy_event_id", "status"),
        Index("ix_strategy_event_deliveries_subscription_status", "subscription_id", "status"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    strategy_event_id: Mapped[int] = mapped_column(nullable=False)
    subscription_id: Mapped[int] = mapped_column(nullable=False)
    channel: Mapped[str] = mapped_column(Text, default="email", server_default="email", nullable=False)
    idempotency_key: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(Text, default="pending", server_default="pending", nullable=False)
    attempts: Mapped[int] = mapped_column(default=0, server_default=text("0"), nullable=False)
    provider_message_id: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
