...
PROVIDER_DOMAIN_DEFAULTS: tuple[ProviderDomainDefault, ...] = (
    ...
    _domain("macro_positioning", "Macro Positioning", "internal_computed", "walnut_cache", "primary", "internal computed", "safe", ("macro_positioning_weekly_refresh",), "macro_positioning_cache", allowed_providers=INTERNAL_COMPUTED_PROVIDERS, allowed_fallbacks=INTERNAL_COMPUTED_FALLBACKS, allowed_modes=("primary", "disabled"), notes="Weekly institutional macro positioning is normalized, interpreted, cached, and served as Walnut intelligence."),
    ...
    _domain(
        "insights_us_sectors",
        "Insights: sector performance snapshot",
        "fmp",
        "walnut_cache",
        "primary",
        "external API",
        "safe",
        ("sector-performance-snapshot", "sector ETF proxy price_cache"),
        "insights_snapshots",
        primary_endpoint_url=f"{FMP_STABLE_BASE_URL}/sector-performance-snapshot?date={{date}}",
        allowed_providers=MARKET_PROVIDERS,
        allowed_fallbacks=MARKET_FALLBACKS,
        allowed_modes=MARKET_MODES,
        allow_external_live_fetch=True,
        allow_user_route_sync_fetch=True,
        notes="US sector expanded snapshots use FMP sector-performance-snapshot; cached ETF proxies are fallback only.",
    ),
