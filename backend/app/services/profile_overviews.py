...
def _congress_market_analytics(db: Session, *, since: datetime, chamber: str) -> dict[str, Any]:
    ...
    for ts, symbol, trade_type, transaction_type, amount_min, amount_max, member_id, row_chamber in rows:
        ...
        if ts >= current_since:
            values["buy" if buy else "sell"] += value; values["trades"] += 1; values["trend"][date(ts.year, ts.month, 1).strftime("%b %y")] += value if buy else -value; chambers[(str(row_chamber or "Unknown").title() or "Unknown")] += value; month = date(ts.year, ts.month, 1)
            if month in monthly:
    ...
    activity = [{"sector": sector, "current_value": _float_or_int(value["buy"] - value["sell"]) or 0, "previous_value": _float_or_int(value["prior_buy"] - value["prior_sell"]) or 0, "change_pct": _pct(value["buy"] - value["sell"], value["prior_buy"] - value["prior_sell"]), "buy_value": _float_or_int(value["buy"]) or 0, "sell_value": _float_or_int(value["sell"]) or 0, "trades": int(value["trades"] or 0), "trade_percent": round((int(value["trades"] or 0) / total_sector_trades) * 100, 1) if total_sector_trades else 0, "trend": [{"label": month.strftime("%b %y"), "value": _float_or_int(value["trend"].get(month.strftime("%b %y"), 0)) or 0} for month in months]} for sector, value in sector_data.items()]; activity.sort(key=lambda row: abs(float(row["current_value"])), reverse=True)
