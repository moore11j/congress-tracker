...
class Recorder:
 ...
 def frame(self):
  ...
  masks=self.page.evaluate('''() => {
   const out=[];
   const add=e=>{const b=e.getBoundingClientRect();if(b.width&&b.height&&b.bottom>0&&b.top<1000)out.push({x:b.x,y:b.y,width:b.width,height:b.height});};
 ...
 def move(self,loc,steps=7):
  ...
  if not 0<=end[1]<=1000:raise ValueError('Click target is outside the visible navigation.')
 ...
 def click(self,loc,label):
  self.move(loc);self.clicked=True;self.frame()
 ...
 def circle(self,loc):
  b=self.box(loc);cx=b['x']+b['width']/2;cy=b['y']+b['height']/2
 ...
 def scroll_to(self,loc,top=150,steps=20):
  ...
  self.pointer=(890,620);self.page.mouse.move(*self.pointer)
  previous=0
  for i in range(1,steps+1):
 ...
 with tempfile.TemporaryDirectory(prefix='walnut-navigation-') as folder,sync_playwright() as pw:
  ...
  def guard(route):
   ...
   if p.scheme in {'http','https'}:
    ...
    if p.path.startswith('/api/') and not (p.path in {'/api/auth/me','/api/entitlements'} or p.path.startswith(('/api/tickers/NVDA','/api/tickers/SPY','/api/institutions/0001462245','/api/search','/api/research','/api/insights','/api/market','/api/macro'))):return route.abort()
  ...
  def position(loc,top=150):
   loc.evaluate("(e,top)=>window.scrollBy(0,e.getBoundingClientRect().top-top)",top)
  ...
  if shot=='v4_search':
   ...
   manager.wait_for(timeout=60000);position(activity)
   ...
   r.circle(briefs.get_by_role('heading',name='RESEARCH BRIEFS',exact=True))
  else:
