...
DEFAULT_TEMPLATES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "name": "AI outreach digest",
        "category": "admin",
        "from_name": ALERTS_FROM_NAME,
        "from_email": ALERTS_FROM_EMAIL,
        "reply_to": SUPPORT_EMAIL,
        "subject": "{{digest_title}}",
        "preheader": "Human-reviewed Walnut outreach reply candidates.",
        "variables": ["first_name", "digest_title", "summary", "items_text", "items_html", "digest_url"],
        "body_text": walnut_email_text(
            greeting="Hello {{first_name}},",
            intro="{{summary}}",
            sections=[
                "Review each reply before posting. Avoid spam and disclose affiliation when relevant.",
                "{{items_text}}",
            ],
            cta_label="Open AI Outreach",
            cta_url="digest_url",
            sender=ALERTS_FROM_NAME,
            include_investment_disclaimer=True,
        ),
        "body_html": walnut_email_html(
            ...
            content_html=walnut_info_card(
                "Review guardrail",
                "Review each reply before posting. Avoid spam and disclose affiliation when relevant. No auto-posting was performed.",
            )
            + "{{{items_html}}}",
            cta_label="Open AI Outreach",
            cta_url="digest_url",
