"""Read-only third hash block. The final block is deliberately reserved."""
import base64
import hashlib
import json
import zlib
...
with SessionLocal() as db:
    ...
    symbols = sorted(set(ordered[512:768]+['SPY','QQQ']))
    ...
    payload = {'eligibility':'Third fixed hash block 512:768; final block reserved', 'eligible_count':len(eligible), 'reserved_count':len(ordered[768:]), 'symbols':symbols, 'columns':['symbol','date','raw_close','adjusted_close','open','volume','source','status','split'], 'rows':[list(r) for r in db.execute(q,{'symbols':symbols})]}
