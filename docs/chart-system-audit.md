...
2. In progress: profile performance and institutional allocation. Congress performance now uses the shared binary nearest-point lookup, RAF hover scheduling, reveal clipping, keyboard support, and touch scrubbing; the allocation donut now supports touch selection and reduced-motion-safe entry. Generic profile and government line trends, dashboard mix pie charts, and Congress/insider/institution/government activity charts now use shared interactive primitives. The Congress, insider, and institutional dashboards' headline metric trends, sector exposure, and net sector activity now animate and support chart or row-level inspection. Profile-card mini charts now reveal source-backed prior-period and latest-period comparisons, with explicit x/y axes and an elevated value tooltip; profile landing sector rows animate and expose source-backed current/prior activity on demand. Department spending mix now reveals on entry, respects reduced motion, and exposes selected-period sector percentages through hover, touch, and keyboard focus.
