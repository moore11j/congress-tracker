...
import { ApiError, getTickerOperationalIntelligence, type OperationalIntelligenceItem, type TickerOperationalIntelligence } from "@/lib/api";
...
export function TickerOperationalIntelligenceCard({ symbol }: { symbol: string }) {
  const [data, setData] = useState<TickerOperationalIntelligence | null>(null);
  ...
  useEffect(() => {
    if (!enabled) return;
    let active = true;
    setData(null); setFailed(false); setDisabled(false);
    getTickerOperationalIntelligence(symbol).then((value) => { if (active) setData(value); }).catch((error) => { if (active) { if (error instanceof ApiError && error.status === 404) setDisabled(true); else setFailed(true); } });
    return () => { active = false; };
  }, [enabled, symbol, retry]);
