...
import { useResearchMemoryAccess } from "@/components/research-memory/ResearchMemoryAccess";
...
export function TickerOperationalIntelligenceProvider({ symbol, children }: { symbol: string; children: ReactNode }) {
  const { status: access } = useResearchMemoryAccess();
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [symbol, enabled, access, attempt]);
  const current = access !== "allowed" ? { data: null, failed: false, disabled: true } : state.symbol === symbol ? state : { data: null, failed: false, disabled: !enabled };
