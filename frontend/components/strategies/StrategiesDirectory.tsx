...
export function StrategiesDirectory({ data, featured, category, period, sort }: Props) {
  ...
  return <div className="space-y-3 py-3 sm:py-5">
    ...
    {items.length ? <div className="grid gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(19rem,0.9fr)]">
      ...
      <aside className="space-y-4"><section className="rounded-lg border border-white/10 bg-slate-950/35 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-300">Selected strategy</p><h2 className="mt-2 text-xl font-semibold text-white">{selected ? displayStrategyName(selected.name) : "No strategy selected"}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{selected?.shortDescription ?? selected?.walnutTake ?? "Choose a published strategy to inspect its stored record."}</p></div>{selected ? <StatusPill strategy={selected} /> : null}</div>{selected ? <dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-4 text-sm"><div><dt className="text-slate-500">Universe</dt><dd className="mt-1 text-slate-200">{displayStrategyUniverse(selected)}</dd></div><div><dt className="text-slate-500">Benchmark</dt><dd className="mt-1 text-slate-200">{selected.latestRun?.benchmark ?? benchmark}</dd></div><div><dt className="text-slate-500">Start date</dt><dd className="mt-1 text-slate-200">{formatDate(selected.latestRun?.backtestStartDate)}</dd></div><div><dt className="text-slate-500">Current status</dt><dd className="mt-1 text-slate-200">{displayStatus(selected)}</dd></div></dl> : null}<Link href={selected ? `/strategies/${selected.slug}` : "/strategies/methodology"} className="mt-5 inline-flex rounded-md border border-emerald-300/30 px-3 py-2 text-sm font-semibold text-emerald-100 hover:bg-emerald-300/10">{selected ? "Open strategy" : "Review methodology"}</Link></section>
