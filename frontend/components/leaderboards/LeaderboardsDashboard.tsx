...
import { partyBadge } from "@/lib/format";
...
type Snapshot = { items: Record<string, unknown>[]; locked_ranks?: number[]; preview?: boolean; filter_items?: Record<string, Record<string, unknown>[]>; generated_at: string | null; timeframe_label?: string; methodology?: string; empty_message?: string };
...
function partyTextClass(party: string) {
  const { tone } = partyBadge(party);
  if (tone === "dem") return "text-blue-400";
  if (tone === "rep") return "text-red-400";
...
function Locked({ title, detail, tier, feature }: { title: string; detail: string; tier: "Premium" | "Pro"; feature: string }) {
  return <div className="mt-4"><ContextualUpgrade title={title} body={detail} tier={tier} feature={feature} /></div>;
}
function PreviewGate({ tier, label }: { tier: "Premium" | "Pro"; label: string }) {
  const stocks = label.includes("Top Stocks");
  const feature = stocks ? "top_stocks" : label.includes("Congress") ? "congress_leaderboard" : label.includes("insider") ? "insider_leaderboard" : "institutional_leaderboard";
  return <div className="mt-3"><ContextualUpgrade title={stocks ? "See all top-ranked stocks" : "See the full ranking"} body={stocks ? "Unlock the full leaderboard, Confirmation Scores, and the data driving each ranking." : "Unlock every ranked participant, historical performance metrics, and the activity behind the results."} tier={tier} feature={feature} /></div>;
}
...
function Discovery({snapshot, stocks = false, tier = "Premium"}: {snapshot: Snapshot | null; stocks?: boolean; tier?: "Premium" | "Pro"}) {
  const guest = snapshot?.locked_ranks?.length !== 0;
  return <div className="mt-4"><p className="text-sm text-slate-400">{guest ? "Ranks #3–#5 are a public preview. Create a free account to unlock #1 and #2." : "Your Top 5 are unlocked. Explore more rankings and detailed evidence with a paid plan."}</p>
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      {guest && <RankingLocks signupUrl="/login?mode=register&return_to=%2Fwelcome" stocks={stocks} />}
      {(snapshot?.items ?? []).filter(row => (number(row, "rank") ?? 0) >= (guest ? 3 : 1) && (number(row, "rank") ?? 0) <= 5).map(row => <article key={String(row.rank)} className="min-w-0 rounded-lg border border-white/10 p-4"><p className="font-mono text-emerald-300">#{String(row.rank)}</p><h3 className="mt-2 break-words font-semibold text-white">{stocks ? text(row, "symbol") : text(row, "name")}</h3><p className="mt-1 text-sm text-slate-400">{text(row, "company_name")}</p>{stocks && <><p className="mt-3 text-xs font-semibold text-slate-200">Why this ranked</p><p className="mt-1 text-sm text-slate-300">{text(row, "why_ranked")}</p><p className="mt-2 text-xs text-slate-400">{Array.isArray(row.key_drivers) ? row.key_drivers.filter(v => typeof v === "string").join(" · ") : ""}</p><Link href={`/ticker/${encodeURIComponent(text(row, "symbol"))}`} className="mt-3 inline-flex text-sm text-emerald-200">View analysis →</Link></>}</article>)}
    </div>{!snapshot?.items.length && <Empty snapshot={snapshot} />}
    {guest ? <Link href="/login?mode=register&return_to=%2Fwelcome" className="mt-4 inline-flex rounded-lg bg-emerald-300 px-4 py-3 text-sm font-semibold text-slate-950">Unlock the Top 2 · Create Free Account</Link> : <PreviewGate tier={tier} label={stocks ? "Top Stocks" : "ranking"} />}
  </div>;
}

function ScoreCoverage({row}: {row: Record<string, unknown>}) {
  const coverage = row.confirmation_coverage as Record<string, unknown> | undefined;
  const aligned = coverage && number(coverage, "aligned_source_count");
  const total = coverage && number(coverage, "source_count");
  return typeof aligned === "number" && typeof total === "number" ? <p className="mt-1 text-[10px] text-slate-400">{aligned} of {total} sources aligned</p> : null;
}
...
function TopStocks({ snapshot, allowed }: { snapshot: Snapshot; allowed: boolean }) {
  ...
  if (!allowed) return <Panel id="top-stocks" trail="Top Stocks" title="Top Stock Ideas" subtitle="Walnut filters the evidence and ranks the strongest ideas" snapshot={snapshot}><Discovery snapshot={snapshot} stocks /></Panel>;
  ...
  return <Panel id="top-stocks" trail="Top Stocks" title="Top Stocks" subtitle="Stocks ranked by Walnut Confirmation Score" snapshot={snapshot} className="min-w-0" showUpdated={Boolean(snapshot.generated_at)}>
    ...
    {!rows.length ? (allowed ? <Empty snapshot={{ ...snapshot, empty_message: "No qualifying stocks in this prepared daily filter snapshot." }} /> : <Locked title="Premium leaderboard" feature="stock_leaderboard" detail="Top Stocks and every prepared filter are available with Premium." tier="Premium" />) : <><div className="mt-3 overflow-x-auto"><table className="min-w-[730px] w-full text-left text-xs"><thead className="border-y border-white/10 bg-white/[0.025] text-[10px] font-medium text-slate-400"><tr><th className="px-3 py-3">Rank</th><th className="px-3 py-3">Ticker</th><th className="px-3 py-3">Company</th><th className="px-3 py-3">Confirmation score</th><th className="px-3 py-3">Direction</th><th className="px-3 py-3">Price</th><th className="px-3 py-3">Action</th></tr></thead><tbody className="divide-y divide-white/10">{visibleRows.map((row) => {
      const symbol = text(row, "symbol"); const href = text(row, "ticker_url") || `/ticker/${symbol}`; const score = number(row, "confirmation_score");
      return <tr key={symbol} className="hover:bg-white/[0.025]"><td className="px-3 py-3 font-mono text-sm font-semibold text-white">{number(row, "rank") ?? "—"}</td><td className="px-3 py-3"><Link href={href} onClick={() => track("leaderboard_stock_click", { symbol })} className="font-mono text-sm font-semibold text-white hover:text-emerald-200">{symbol}</Link></td><td className="max-w-[18rem] px-3 py-3 text-slate-300">{text(row, "company_name")}<p className="mt-1 text-xs text-slate-400">{text(row, "why_ranked")}</p><details className="mt-2"><summary className="cursor-pointer text-emerald-200">Why this ranked</summary><p className="mt-2 text-xs">{Array.isArray(row.key_drivers) ? row.key_drivers.join(" · ") : ""}</p>{Array.isArray(row.why_this_ranked) && row.why_this_ranked.map((value, i) => { const evidence = value as Record<string, unknown>; return <p key={i} className="mt-2 text-xs leading-5"><strong>{text(evidence, "source")}</strong>: {text(evidence, "summary") || text(evidence, "direction")}</p>; })}<Link href={href} className="mt-2 inline-flex text-emerald-200">Full evidence and historical setups →</Link></details></td><td className="px-3 py-3">{allowed ? <div className="flex items-center gap-2"><span className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-700"><span className="block h-full rounded-full bg-gradient-to-r from-emerald-300 to-emerald-100" style={{ width: `${Math.min(Math.max(score ?? 0, 0), 100)}%` }} /></span><span className="font-mono text-emerald-100">{score ?? "—"}</span></div> : <span className="inline-flex items-center gap-1 rounded border border-amber-300/20 bg-amber-300/10 px-2 py-1 text-[10px] font-semibold text-amber-100" aria-label="Confirmation score requires Premium"><span aria-hidden="true">🔒</span> Premium</span>}{allowed && <ScoreCoverage row={row} />}</td><td className="px-3 py-3"><Direction row={row} /></td><td className="px-3 py-3 font-mono text-slate-200">{price(number(row, "price"))}</td><td className="px-3 py-3"><Link href={href} onClick={() => track("leaderboard_stock_click", { symbol })} className="whitespace-nowrap rounded-md border border-white/15 px-2 py-1.5 font-semibold text-slate-100 hover:border-emerald-300/50 hover:text-emerald-100">View analysis</Link></td></tr>;
    })}</tbody></table></div>{!allowed ? <PreviewGate tier="Premium" label="Unlock the full Top Stocks ranking and filters" /> : <p className="mt-3 text-[11px] text-slate-500">Scores use a fixed, weighted 100-point source model. Quiet or mixed sources earn zero; opposing evidence subtracts. Top 10 is a relative ranking: developing setups can rank below the 60-point strong threshold.</p>}</>}
  </Panel>;
}
...
function Congress({ snapshot, allowed }: { snapshot: Snapshot | null; allowed: boolean }) {
  ...
  if (!allowed) return <Panel id="congress" trail="Congress" title="Top Congress Investors" subtitle="Ranked historical participants" snapshot={snapshot}><Discovery snapshot={snapshot} /></Panel>;
  const visibleRows = rows;
  ...
  return <Panel id="congress" trail="Congress" title="Top Congress Investors" subtitle="Congress members ranked by historical portfolio performance" snapshot={snapshot} action={allowed ? select : <span className="rounded-md border border-emerald-300/25 bg-emerald-300/10 px-2 py-1.5 text-[10px] font-semibold text-emerald-100">Premium</span>} className="min-w-0" showUpdated={Boolean(snapshot?.generated_at)}>
    {!rows.length ? (allowed ? <Empty snapshot={snapshot} /> : <Locked title="Premium leaderboard" feature="congress_leaderboard" detail="Congress portfolio rankings are available with Premium." tier="Premium" />) : <><div className="mt-3 overflow-x-auto"><table className="min-w-[680px] w-full text-left text-xs"><thead className="border-y border-white/10 bg-white/[0.025] text-[10px] text-slate-400"><tr><th className="px-3 py-3">Rank</th><th className="px-3 py-3">Member</th><th className="px-3 py-3">Party</th><th className="px-3 py-3">Total return</th><th className="px-3 py-3">CAGR</th><th className="px-3 py-3">Alpha</th><th className="px-3 py-3">Max drawdown</th></tr></thead><tbody className="divide-y divide-white/10">{visibleRows.map((row, index) => <tr key={text(row, "href") || text(row, "name")} className="hover:bg-white/[0.025]"><td className="px-3 py-3 font-mono text-white">{number(row, "rank") ?? "—"}</td><td className="px-3 py-3"><Link href={text(row, "href")} onClick={() => track("leaderboard_member_click", { name: text(row, "name") })} className="font-medium text-white hover:text-emerald-200">{text(row, "name")}</Link><p className="text-[10px] text-slate-500">{text(row, "chamber")}</p></td><td className={`px-3 py-3 ${partyTextClass(text(row, "party"))}`}>{text(row, "party") || "—"}</td><td className="px-3 py-3 font-mono text-emerald-200">{percent(number(row, "total_return_pct"))}</td><td className="px-3 py-3 font-mono text-emerald-200">{percent(number(row, "cagr_pct"))}</td><td className="px-3 py-3 font-mono text-emerald-200">{percent(number(row, "alpha_pct"))}</td><td className="px-3 py-3 font-mono text-slate-300">{percent(number(row, "max_drawdown_pct"))}</td></tr>)}</tbody></table></div>{!allowed ? <PreviewGate tier="Premium" label="Unlock the complete Congress investor ranking" /> : null}</>}
  </Panel>;
}

function Insiders({ snapshot, allowed }: { snapshot: Snapshot | null; allowed: boolean }) {
  if (!allowed) return <Panel id="insiders" trail="Insiders" title="Top Insiders" subtitle="Ranked historical participants" snapshot={snapshot}><Discovery snapshot={snapshot} /></Panel>;
