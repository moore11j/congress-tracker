...
function TopStocks({ snapshot, allowed }: { snapshot: Snapshot; allowed: boolean }) {
  ...
  return <Panel id="top-stocks" trail="Top Stocks" title="Top Stocks" subtitle="Stocks ranked by Walnut Confirmation Score" snapshot={snapshot} className="min-w-0" showUpdated={allowed}>
    {!allowed ? <Locked title="Premium leaderboard" detail="Top Stocks and every prepared filter are available with Premium." tier="Premium" /> : <><div className="mt-4 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{stockFilters.map(([key, label]) => <button key={key} type="button" aria-pressed={activeFilter === key} onClick={() => { setActiveFilter(key); track("leaderboard_stock_filter_change", { filter: key }); }} className={`shrink-0 rounded-md border px-2.5 py-1.5 text-[11px] transition ${activeFilter === key ? "border-emerald-300/45 bg-emerald-300/10 text-emerald-100" : "border-white/10 bg-slate-950/50 text-slate-400 hover:border-emerald-300/30 hover:text-slate-200"}`}>{label}</button>)}</div>
