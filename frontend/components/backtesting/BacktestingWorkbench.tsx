...
export function BacktestingWorkbench({ initialEntitlements, initialPresets, initialQuery }: Props) {
  ...
  const [lookbackDays, setLookbackDays] = useState<number>(parsePositiveInt(undefined, presets.defaults.lookback_days));
  const [holdDays, setHoldDays] = useState<30 | 60 | 90 | 180 | 365>(presets.defaults.hold_days);
  const [startBalanceInput, setStartBalanceInput] = useState<string>(String(presets.defaults.start_balance));
  const [contributionAmountInput, setContributionAmountInput] = useState<string>(String(presets.defaults.contribution_amount));
  const [contributionFrequency, setContributionFrequency] = useState<BacktestContributionFrequency>(presets.defaults.contribution_frequency);
  const [rebalancingFrequency, setRebalancingFrequency] = useState<BacktestRebalancingFrequency>(presets.defaults.rebalancing_frequency);
  const [result, setResult] = useState<BacktestRunResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (initialPresets.access.signed_in && initialEntitlements.user) return;
    let cancelled = false;
    Promise.all([getEntitlements(), getBacktestPresets()])
      ...
      .catch((refreshError) => {
        console.error("[backtesting] protected client refresh failed", refreshError);
      });
    return () => {
      cancelled = true;
    };
  }, [initialEntitlements.user, initialPresets.access.signed_in]);
