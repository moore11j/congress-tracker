    ...
    <div className="space-y-8">
      <section className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
        ...
        <div className={`${cardClassName} space-y-4`}>
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Results</p><h2 className="mt-1 text-xl font-semibold text-white">Strategy vs S&amp;P 500</h2><p className="mt-2 max-w-2xl text-sm text-slate-400">This is a capital-constrained portfolio simulation. Individual trade returns may be large, but portfolio performance is based on actual allocated capital over time.</p><p className="mt-2 max-w-2xl text-sm text-slate-400">Portfolio math uses dollar/share simulation with total exposure capped at 100% and max {maxPositionWeightPct} per position.</p></div><span className="rounded-full border border-white/10 bg-slate-950/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">{initialEntitlements.tier}</span></div>
