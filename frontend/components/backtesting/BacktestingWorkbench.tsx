...
export function BacktestingWorkbench({ initialEntitlements, initialPresets, initialQuery }: Props) {
  ...
  const payload = useMemo<BacktestRunRequest | null>(() => {
    ...
    if (strategy === "congress") {
      ...
      return { ...base, source_scope: (sourceScope === "house" || sourceScope === "senate" || sourceScope === "member" ? sourceScope : "all_congress"), member_id: memberId.trim() || undefined };
    }
      ...
      <section className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
        <div className={`${cardClassName} space-y-5`}>
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-400">Run capital-constrained historical simulations across watchlists, saved screens, Congress filings, and insider disclosures.</p>
          </div>
        ...
        <div className={`${cardClassName} space-y-4`}>
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Results</p><h2 className="mt-1 text-xl font-semibold text-white">Strategy vs S&amp;P 500</h2><p className="mt-2 max-w-2xl text-sm text-slate-400">This is a capital-constrained portfolio simulation. Individual trade returns may be large, but portfolio performance is based on actual allocated capital over time.</p></div><span className="rounded-full border border-white/10 bg-slate-950/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">{initialEntitlements.tier}</span></div>
          {loading ? <ResultSkeleton /> : error ? <div className="space-y-4"><div className="rounded-2xl border border-rose-300/20 bg-rose-400/[0.07] px-4 py-3 text-sm text-rose-100">{error}</div><div className="rounded-2xl border border-dashed border-white/10 px-5 py-10 text-center"><h3 className="text-lg font-semibold text-white">Backtest run failed</h3><p className="mt-2 text-sm text-slate-400">Review the selected inputs and try again.</p></div></div> : !canRun ? <div className="space-y-4"><UpgradePrompt title="Unlock portfolio backtesting" body="Backtesting is included with Premium. Free users can preview the workflow here, but custom runs stay locked until upgrade." /><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{["Ending Balance", "Net Profit", "Total Return", "CAGR", "Alpha", "Sharpe Ratio"].map((label) => <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3"><p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">{label}</p><p className="mt-3 text-2xl font-semibold text-slate-300">Premium</p></div>)}</div><div className="rounded-2xl border border-white/10 bg-[#07111d] px-4 py-12 text-center text-sm text-slate-400">Portfolio equity curve previews unlock after upgrade.</div></div> : result ? <div className="space-y-5">
