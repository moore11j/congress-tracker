...
export function EnhancedProfilesOverview({ data }: { data: ProfilesSummaryResponse }) {
  ...
  const activityMix = data.activity_mix?.length
    ? new Map(data.activity_mix.map((item) => [item.type, item.value]))
  ...
  return (
    <main className="relative min-w-0 space-y-3 overflow-hidden pb-3">
      ...
      <section className="relative z-10 grid items-stretch gap-3 xl:grid-cols-[0.92fr_2.08fr]">
...
export function EnhancedCongressDashboard({ data, chamberFilter }: { data: CongressOverviewResponse; chamberFilter: ReactNode }) {
  const monthly = (data.monthly_activity ?? []).slice(-12);
  ...
  return <main className="relative min-w-0 space-y-3 overflow-hidden pb-3"><OverviewGlow /><header className="relative z-10 flex flex-col gap-3 pt-2 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.28em] text-emerald-300">Congress</p><h1 className="mt-2 text-3xl font-semibold text-white md:text-4xl">Congress Trading</h1><p className="mt-2 text-sm text-slate-300">Track disclosed trades, portfolio activity, and market positioning across members of Congress.</p></div>{chamberFilter}</header><section className="relative z-10 grid gap-3 xl:grid-cols-[1.7fr_.9fr]"><Panel title="Congress trading snapshot" subtitle="Monthly activity over the last 12 months"><div className="grid gap-4 md:grid-cols-[7rem_minmax(0,1fr)]"><div><p className="text-2xl font-semibold text-white">{formatNumber(Number(snap.total_trades ?? 0))}</p><p className="text-[10px] uppercase tracking-[.14em] text-slate-500">Total trades</p><MetricDelta metric={metrics("Total Trades")} /></div><CongressSnapshotTrend points={monthly.map((row) => ({ label: row.period, value: row.trades }))} /></div></Panel><div className="grid gap-3 sm:grid-cols-2"><CongressSnapshot icon="member" tone="green" label="Most active member" row={snap.top_member as Row} valueKey="trades" format="number" suffix=" trades" /><CongressSnapshot icon="ticker" tone="purple" label="Most traded ticker" row={snap.most_traded_ticker as Row} valueKey="net_value" suffix=" Net Value" /><CongressSnapshot icon="buyer" tone="green" label="Top buyer" row={snap.top_buyer as Row} valueKey="value" suffix=" Net Buys" /><CongressSnapshot icon="sector" tone="amber" label="Most active sector" row={snap.most_active_sector as Row} valueKey="trade_percent" format="percent" suffix=" of Trades" /></div></section><section className="relative z-10 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{data.summary.map((metric) => <CongressMetric key={metric.label} metric={metric} monthly={monthly} />)}</section><section className="relative z-10 grid gap-3 xl:grid-cols-2"><RankTable title="Top members by portfolio value" rows={data.top_members} columns={[["name", "Member", "link"], ["party", "Party"], ["chamber", "Chamber"], ["estimated_portfolio_value", "Est. portfolio value", "currency"], ["trades", "Trades", "number"], ["recent_activity", "Recent activity", "date"]]} /><RankTable title="Most traded stocks" rows={data.most_traded_stocks} columns={[["symbol", "Ticker", "link"], ["company", "Company"], ["buy_value", "Buy value", "currency"], ["sell_value", "Sell value", "currency"], ["net_value", "Net value", "signed_currency"], ["trades", "Trades", "number"]]} /></section><section className="relative z-10 grid gap-3 xl:grid-cols-[1.05fr_.85fr_.8fr]"><VerticalSectorExposure rows={data.sector_exposure} /><SignedSectorBars rows={data.sector_activity ?? []} /><Panel title="Buy vs sell mix"><div className="flex items-center gap-4"><Donut values={mix} colors={["#42d3a7", "#fb7185"]} label="Buy" value={`${mix[0] + mix[1] ? ((mix[0] / (mix[0] + mix[1])) * 100).toFixed(0) : 0}%`} /></div></Panel></section><section className="relative z-10 grid gap-3 xl:grid-cols-[.85fr_1fr_1.15fr]"><ChamberMix rows={data.chamber_mix ?? []} /><CongressMovers rows={data.top_moving_sectors ?? []} /><NotableTrades items={data.recent_notable_trades ?? data.recent_disclosures} /></section><section className="relative z-10"><Panel title="Activity over time" action="Monthly"><CongressActivityOverTime rows={monthly} /></Panel></section></main>;
}
...
function CongressSnapshotTrend({ points }: { points: Array<{ label: string; value: number }> }) { if (!points.length) return <p className="flex h-48 items-center justify-center text-sm text-slate-400">No monthly trade activity is available.</p>; const max = Math.max(...points.map((point) => point.value), 1); const axisMax = roundAxisMax(max); const ticks = [axisMax, axisMax * .75, axisMax * .5, axisMax * .25, 0]; const plotTop = 8; const plotBottom = 92; const xFor = (index: number) => (index / Math.max(points.length - 1, 1)) * 100; const yFor = (value: number) => plotBottom - (Math.max(value, 0) / axisMax) * (plotBottom - plotTop); const linePoints = points.map((point, index) => `${xFor(index)},${yFor(point.value)}`); const line = linePoints.join(" "); const area = `M${linePoints.join(" L ")} L ${xFor(points.length - 1)},${plotBottom} L ${xFor(0)},${plotBottom} Z`; const diamondPoints = (x: number, y: number) => `${x},${y - 1.15} ${x + .26},${y} ${x},${y + 1.15} ${x - .26},${y}`; return <div className="min-w-0"><div className="grid grid-cols-[3rem_minmax(0,1fr)] gap-3"><div className="flex h-48 flex-col justify-between py-1 text-right text-[10px] font-semibold tabular-nums text-slate-500">{ticks.map((tick) => <span key={tick}>{formatCompactNumber(tick)}</span>)}</div><div className="min-w-0 overflow-hidden"><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-48 w-full overflow-hidden border-b border-white/10" aria-label="Monthly Congress trade count trend">{ticks.map((tick) => <line key={tick} x1="0" x2="100" y1={yFor(tick)} y2={yFor(tick)} stroke="rgba(148,163,184,.16)" vectorEffect="non-scaling-stroke" />)}<path d={area} fill="url(#congress-trades-area)" /><polyline points={line} fill="none" stroke="#55e3b0" strokeWidth="1.8" vectorEffect="non-scaling-stroke" />{points.map((point, index) => <polygon key={`${point.label}-diamond`} points={diamondPoints(xFor(index), yFor(point.value))} fill="#55e3b0" stroke="#07101f" strokeWidth=".35" vectorEffect="non-scaling-stroke" />)}<defs><linearGradient id="congress-trades-area" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#42d3a7" stopOpacity=".38" /><stop offset="100%" stopColor="#42d3a7" stopOpacity=".04" /></linearGradient></defs></svg><div className="mt-2 grid gap-1 text-[10px] text-slate-500" style={{ gridTemplateColumns: `repeat(${points.length}, minmax(0, 1fr))` }}>{points.map((point) => <span key={point.label} className="truncate text-center">{point.label}</span>)}</div></div></div></div>; }
function CongressActivityOverTime({ rows }: { rows: CongressPoint[] }) { const points = rows.slice(-12).map((row) => ({ label: row.period, buy: Number(row.buy_value ?? 0), sell: Number(row.sell_value ?? 0), trades: Number(row.trades ?? 0) })); if (!points.length) return <p className="flex h-64 items-center justify-center text-sm text-slate-400">No monthly Congress activity is available.</p>; const valueLimit = roundAxisMax(Math.max(...points.flatMap((point) => [point.buy, point.sell]), 1)); const valueTicks = [valueLimit, valueLimit * 2 / 3, valueLimit / 3, 0, -valueLimit / 3, -valueLimit * 2 / 3, -valueLimit]; const tradeLimit = roundAxisMax(Math.max(...points.map((point) => point.trades), 1)); const tradeTicks = [tradeLimit, tradeLimit * 5 / 6, tradeLimit * 4 / 6, tradeLimit * 3 / 6, tradeLimit * 2 / 6, tradeLimit / 6, 0]; const plotTop = 8; const plotBottom = 92; const zeroY = 50; const xFor = (index: number) => 5 + (index / Math.max(points.length - 1, 1)) * 90; const yValue = (value: number) => zeroY - (Math.max(-valueLimit, Math.min(valueLimit, value)) / valueLimit) * ((plotBottom - plotTop) / 2); const yTrades = (value: number) => plotBottom - (Math.max(value, 0) / tradeLimit) * (plotBottom - plotTop); const barWidth = Math.min(3.4, 24 / points.length); const tradeLine = points.map((point, index) => `${xFor(index)},${yTrades(point.trades)}`).join(" "); const minChartWidth = Math.max(46, points.length * 4.7); return <div className="min-w-0"><div className="mb-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-slate-400"><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm bg-emerald-400" />Buy Value (USD)</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm bg-rose-400" />Sell Value (USD)</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-blue-400" />Total Trades</span></div><div className="grid grid-cols-[3.8rem_minmax(0,1fr)_3.6rem] gap-3"><div><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">Value</p><div className="flex h-64 flex-col justify-between text-right text-[10px] font-semibold tabular-nums text-slate-500">{valueTicks.map((tick) => <span key={tick}>{formatAxisValue(tick)}</span>)}</div></div><div className="min-w-0 overflow-x-auto pb-1"><div style={{ minWidth: `${minChartWidth}rem` }}><div className="relative h-64 overflow-hidden"><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full overflow-hidden border-b border-white/10" aria-label="Monthly Congress buy value, sell value, and total trades">{valueTicks.map((tick) => <line key={tick} x1="0" x2="100" y1={yValue(tick)} y2={yValue(tick)} stroke="rgba(148,163,184,.16)" vectorEffect="non-scaling-stroke" />)}<line x1="0" x2="100" y1={zeroY} y2={zeroY} stroke="rgba(148,163,184,.3)" vectorEffect="non-scaling-stroke" />{points.map((point, index) => { const x = xFor(index); const buyY = yValue(point.buy); const sellY = yValue(-point.sell); return <g key={point.label}><rect x={x - barWidth - .35} y={buyY} width={barWidth} height={zeroY - buyY} rx=".45" fill="#42d3a7" opacity=".78" /><rect x={x + .35} y={zeroY} width={barWidth} height={sellY - zeroY} rx=".45" fill="#fb7185" opacity=".78" /></g>; })}<polyline points={tradeLine} fill="none" stroke="#60a5fa" strokeWidth="1.8" vectorEffect="non-scaling-stroke" /></svg>{points.map((point, index) => <span key={`${point.label}-congress-trade-dot`} className="absolute h-2 w-2 rounded-full border border-blue-100 bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,.75)]" style={{ left: `calc(${xFor(index)}% - 4px)`, top: `calc(${yTrades(point.trades)}% - 4px)` }} />)}</div><div className="mt-2 grid gap-1 text-[10px] text-slate-500" style={{ gridTemplateColumns: `repeat(${points.length}, minmax(0, 1fr))` }}>{points.map((point) => <span key={point.label} className="truncate text-center">{point.label}</span>)}</div></div></div><div><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">Trades</p><div className="flex h-64 flex-col justify-between text-left text-[10px] font-semibold tabular-nums text-slate-500">{tradeTicks.map((tick) => <span key={tick}>{formatCompactNumber(tick)}</span>)}</div></div></div></div>; }
...
function CongressTrend({ points, tone = "green" }: { points: Array<{ label: string; value: number }>; tone?: "green" | "red" | "blue" }) { const max = Math.max(...points.map((point) => point.value), 1); const plotTop = 14; const plotBottom = 96; const xFor = (index: number) => (index / Math.max(points.length - 1, 1)) * 100; const yFor = (value: number) => plotBottom - (Math.max(value, 0) / max) * (plotBottom - plotTop); const linePoints = points.map((point, index) => `${xFor(index)},${yFor(point.value)}`); const line = linePoints.join(" "); const area = `M${linePoints.join(" L ")} L ${xFor(points.length - 1)},${plotBottom} L ${xFor(0)},${plotBottom} Z`; const stroke = tone === "red" ? "#fb7185" : tone === "blue" ? "#60a5fa" : "#55e3b0"; const gradientId = `congress-metric-area-${tone}`; return <div><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-36 w-full overflow-hidden">{[25, 50, 75].map((y) => <line key={y} x1="0" x2="100" y1={y} y2={y} stroke="rgba(148,163,184,.15)" vectorEffect="non-scaling-stroke" />)}<path d={area} fill={`url(#${gradientId})`} /><polyline points={line} fill="none" stroke={stroke} strokeWidth="1.7" vectorEffect="non-scaling-stroke" /><defs><linearGradient id={gradientId} x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor={stroke} stopOpacity=".32" /><stop offset="100%" stopColor={stroke} stopOpacity=".04" /></linearGradient></defs></svg><div className="mt-2 flex justify-between text-[10px] text-slate-500"><span>{points[0]?.label}</span><span>{points.at(-1)?.label}</span></div></div>; }
type CongressSnapshotIconName = "member" | "ticker" | "buyer" | "sector";
type CongressSnapshotTone = "green" | "purple" | "amber";
function CongressSnapshot({ icon, tone, label, row, valueKey, suffix = "", format = "currency" }: { icon: CongressSnapshotIconName; tone: CongressSnapshotTone; label: string; row?: Row; valueKey: string; suffix?: string; format?: "currency" | "number" | "percent" }) { const title = row ? stringAt(row, ["name", "symbol", "sector", "company"]) ?? "No reported record" : "No reported record"; const value = row?.[valueKey]; const display = typeof value === "number" ? `${formatCongressSnapshotValue(value, format)}${suffix}` : "-"; const toneClass = tone === "purple" ? "border-violet-300/35 bg-violet-400/15 text-violet-200 shadow-[0_0_18px_rgba(168,85,247,.16)]" : tone === "amber" ? "border-amber-300/35 bg-amber-400/15 text-amber-200 shadow-[0_0_18px_rgba(246,185,26,.16)]" : "border-emerald-300/35 bg-emerald-400/15 text-emerald-200 shadow-[0_0_18px_rgba(66,211,167,.16)]"; return <div className="rounded-lg border border-slate-700/70 bg-slate-950/65 p-4"><div className="flex items-center gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border ${toneClass}`}><CongressSnapshotIcon name={icon} /></span><p className="text-[10px] uppercase tracking-[.14em] text-slate-500">{label}</p></div><p className="mt-3 truncate text-sm font-semibold text-white">{title}</p><p className="mt-1 text-xs font-semibold text-emerald-300">{display}</p></div>; }
function CongressSnapshotIcon({ name }: { name: CongressSnapshotIconName }) { const common = { fill: "none", stroke: "currentColor", strokeLinecap: "round" as const, strokeLinejoin: "round" as const, strokeWidth: 1.8 }; const paths: Record<CongressSnapshotIconName, ReactNode> = { member: <><circle cx="10" cy="7" r="3" {...common} /><path d="M4.5 16.5c1-3.4 2.8-5.1 5.5-5.1s4.5 1.7 5.5 5.1" {...common} /></>, ticker: <><path d="M4 15.5h12" {...common} /><path d="M5 12l3-3 2.5 2.5L15 6" {...common} /><path d="M13 6h2v2" {...common} /></>, buyer: <><path d="M5 15 15 5" {...common} /><path d="M9 5h6v6" {...common} /></>, sector: <><path d="M10 3.5v13" {...common} /><path d="M16.5 10A6.5 6.5 0 1 1 10 3.5" {...common} /><path d="M10 10h6.5" {...common} /></> }; return <svg viewBox="0 0 20 20" aria-hidden="true" className="h-5 w-5">{paths[name]}</svg>; }
function formatCongressSnapshotValue(value: number, format: "currency" | "number" | "percent") { if (format === "number") return formatNumber(value); if (format === "percent") return `${value.toFixed(1)}%`; return formatMoney(value); }
function CongressMetric({ metric, monthly }: { metric: ProfileMetric; monthly: CongressPoint[] }) { const key = metric.label === "Total Trades" ? "trades" : metric.label === "Total Buy Value" ? "buy_value" : metric.label === "Total Sell Value" ? "sell_value" : metric.label === "Active Members" ? "active_members" : "average_trade_size"; const tone = metric.label === "Total Sell Value" ? "red" : metric.label === "Active Members" || metric.label === "Average Trade Size" ? "blue" : metric.label === "Total Trades" && typeof metric.change_pct === "number" && metric.change_pct < 0 ? "red" : "green"; return <div className="rounded-lg border border-slate-700/70 bg-slate-950/65 p-4"><p className="truncate text-[10px] uppercase tracking-[.14em] text-slate-500">{metric.label}</p><p className="mt-3 text-2xl font-semibold text-white">{formatMetric(metric)}</p><MetricDelta metric={metric} /><div className="mt-3"><CongressTrend points={monthly.map((row) => ({ label: row.period, value: Number(row[key] ?? 0) }))} tone={tone} /></div></div>; }
function RankTable({ title, rows, columns }: { title: string; rows: Row[]; columns: Array<[string, string, CellFormat?]> }) { return <Panel title={title}>{!rows.length ? <p className="py-8 text-sm text-slate-400">No reported database records are available.</p> : <div className="overflow-x-auto"><table className="min-w-[48rem] w-full text-left text-xs"><thead className="border-b border-white/10 text-[10px] uppercase tracking-[.13em] text-slate-500"><tr><th className="px-2 py-2">Rank</th>{columns.map(([, label]) => <th key={label} className="px-2 py-2">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/5">{rows.slice(0, 10).map((row, index) => <tr key={index}><td className="px-2 py-2.5 text-slate-500">{index + 1}</td>{columns.map(([key, , format]) => <td key={key} className="max-w-40 truncate px-2 py-2.5 text-slate-300">{renderCell(row, key, format)}</td>)}</tr>)}</tbody></table></div>}</Panel>; }
function VerticalSectorExposure({ rows }: { rows: ProfileSectorPeriod[] }) { const periods = rows.slice(-5); const labels = sectorLabels(periods); return <Panel title="Sector exposure over time" subtitle="Reported trade value"><div className="flex h-48 items-end gap-4 border-b border-white/10 px-2">{periods.map((row) => <div key={row.period} className="flex flex-1 flex-col items-center gap-2"><div className="flex h-40 w-full max-w-10 flex-col-reverse overflow-hidden rounded-sm bg-slate-900">{orderedSegments(row, labels).map((segment) => <i key={segment.label} style={{ height: `${Math.max(segment.percent, .6)}%`, backgroundColor: colorFor(segment.label, labels) }} />)}</div><span className="text-[10px] text-slate-500">{row.period}</span></div>)}</div></Panel>; }
function SignedSectorBars({ rows }: { rows: NonNullable<CongressOverviewResponse["sector_activity"]> }) { const max = Math.max(...rows.map((row) => Math.abs(row.current_value)), 1); return <Panel title="Net activity by sector" subtitle="Green buy net, red sell net"><div className="space-y-3">{rows.slice(0, 8).map((row) => <div key={row.sector} className="grid grid-cols-[7rem_minmax(0,1fr)_4rem] gap-2 text-xs"><span className="truncate text-slate-300">{row.sector}</span><div className="relative h-2.5 rounded bg-slate-800"><i className="absolute top-0 h-full" style={{ left: row.current_value >= 0 ? "50%" : `${50 - Math.abs(row.current_value) / max * 50}%`, width: `${Math.abs(row.current_value) / max * 50}%`, backgroundColor: row.current_value >= 0 ? "#42d3a7" : "#fb7185" }} /></div><span className={row.current_value >= 0 ? "text-right text-emerald-300" : "text-right text-rose-300"}>{formatMoney(row.current_value)}</span></div>)}</div></Panel>; }
function ChamberMix({ rows }: { rows: Array<{ label: string; value: number }> }) { return <Panel title="Chamber mix" subtitle="Reported trade value">{rows.length ? <div className="flex items-center gap-3"><Donut values={rows.map((row) => row.value)} colors={["#3b82f6", "#a855f7", "#94a3b8"]} label="Value" value={formatMoney(rows.reduce((sum, row) => sum + row.value, 0))} /><div className="flex-1 space-y-2">{rows.map((row, index) => <LegendRow key={row.label} label={row.label} color={["#3b82f6", "#a855f7", "#94a3b8"][index]} value={formatMoney(row.value)} />)}</div></div> : <p className="py-8 text-sm text-slate-400">No chamber data available.</p>}</Panel>; }
function CongressMovers({ rows }: { rows: NonNullable<CongressOverviewResponse["top_moving_sectors"]> }) { const visibleRows = rows.slice(0, 6); return <Panel title="Top moving sectors" subtitle="Vs prior 12 months"><div className="overflow-hidden"><div className="grid grid-cols-[minmax(0,1fr)_4.75rem_5.75rem] gap-3 border-b border-white/10 pb-1.5 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500"><span>Sector</span><span className="text-right">Change</span><span>Trend</span></div><div className="divide-y divide-white/5">{visibleRows.map((row) => <div key={row.sector} className="grid min-h-7 grid-cols-[minmax(0,1fr)_4.75rem_5.75rem] items-center gap-3 py-1.5 text-[11px]"><span className="truncate text-slate-300">{row.sector}</span><span className={typeof row.change_pct === "number" && row.change_pct >= 0 ? "text-right font-semibold tabular-nums text-emerald-300" : "text-right font-semibold tabular-nums text-rose-300"}>{typeof row.change_pct === "number" ? `${row.change_pct >= 0 ? "+" : ""}${row.change_pct.toFixed(1)}%` : "-"}</span><CompactSparkline points={row.trend.slice(-12)} positive={typeof row.change_pct !== "number" || row.change_pct >= 0} /></div>)}</div></div></Panel>; }
function CompactSparkline({ points, positive = true }: { points: Array<{ label: string; value: number }>; positive?: boolean }) { if (!points.length) return <span className="block h-5 w-full overflow-hidden rounded-sm bg-slate-950/50" />; const values = points.map((point) => point.value); const min = Math.min(...values); const max = Math.max(...values); const spread = Math.max(max - min, 1); const line = points.map((point, index) => { const x = (index / Math.max(points.length - 1, 1)) * 100; const y = max === min ? 10 : 18 - ((point.value - min) / spread) * 14; return `${x},${y}`; }).join(" "); return <span className="block h-5 w-full overflow-hidden rounded-sm bg-slate-950/40"><svg viewBox="0 0 100 20" preserveAspectRatio="none" className="h-full w-full overflow-hidden"><polyline points={line} fill="none" stroke={positive ? "#55e3b0" : "#fb7185"} strokeWidth="1.6" vectorEffect="non-scaling-stroke" /></svg></span>; }
function NotableTrades({ items }: { items: ProfileActivityItem[] }) { return <Panel title="Recent notable trades">{items.length ? <div className="overflow-x-auto"><table className="min-w-[31rem] w-full text-left text-xs"><thead className="border-b border-white/10 text-[10px] uppercase tracking-[.13em] text-slate-500"><tr>{["Member", "Ticker", "Action", "Value", "Date"].map((label) => <th key={label} className="px-2 py-2">{label}</th>)}</tr></thead><tbody>{items.slice(0, 7).map((item) => { const action = congressTradeAction(item.activity); return <tr key={String(item.id)} className="border-b border-white/5"><td className="px-2 py-2.5 font-semibold text-emerald-200">{item.profile}</td><td className="px-2 py-2.5 text-slate-300">{item.symbol ?? "-"}</td><td className={`px-2 py-2.5 font-semibold ${action.className}`}>{action.label}</td><td className="px-2 py-2.5 text-white">{formatMoney(item.value)}</td><td className="px-2 py-2.5 text-slate-400">{item.time ? formatDate(item.time) : "-"}</td></tr>; })}</tbody></table></div> : <p className="py-8 text-sm text-slate-400">No recent trades available.</p>}</Panel>; }
function congressTradeAction(value: string | null | undefined) { const normalized = String(value ?? "").trim().toLowerCase(); if (normalized.includes("purchase") || normalized === "buy") return { label: "Buy", className: "text-emerald-300" }; if (normalized.includes("sale") || normalized === "sell") return { label: "Sell", className: "text-rose-300" }; return { label: value || "-", className: "text-slate-300" }; }

export function EnhancedInsiderDashboard({ data, periodFilter }: { data: InsidersOverviewResponse; periodFilter: ReactNode }) {
  const sectorBreadth = insiderSectorBreadth(data.sector_activity);
  return <ProfileDashboard
    ...
    snapshotLabel="Net insider activity (USD)"
    snapshotContent={<InsiderSnapshotChart rows={data.monthly_activity ?? []} />}
    netActivityContent={<InsiderNetSectorBars rows={data.sector_net_activity ?? []} />}
    insiderRoleMix={data.role_mix ?? []}
    insiderTopMovingSectors={data.top_moving_sectors ?? []}
...
export function EnhancedInstitutionDashboard({ data, period }: { data: InstitutionsOverviewResponse; period: string }) {
  ...
  return <ProfileDashboard
    ...
    metrics={data.summary}
    primary={{ title: "Top institutions by portfolio value", rows: data.top_institutions, columns: [["name", "Institution", "link"], ["portfolio_value", "Portfolio value", "currency"], ["previous_value", "Previous quarter", "currency"], ["qoq_change", "QoQ change", "signed_percent"], ["positions", "Positions", "number"]] }}
    secondary={{ title: "Top increased positions", rows: data.position_changes, columns: [["symbol", "Ticker", "link"], ["company", "Company"], ["current_value", "Current value", "currency"], ["previous_value", "Previous value", "currency"], ["increase_value", "Increase", "positive_currency"]] }}
...
type CellFormat = "currency" | "signed_currency" | "positive_currency" | "number" | "date" | "percent" | "signed_percent" | "link";
type SnapshotIconName = "buyer" | "ticker" | "cluster" | "sector";
type SnapshotCardSpec = { label: string; row?: Row; icon?: SnapshotIconName; valueKey?: string; valueFormat?: "currency" | "number" | "percent"; suffix?: string; subLabelKey?: string };
...
function ProfileDashboard({ flavor, eyebrow, title, subtitle, filter, comparison, snapshotTitle, snapshotSeries, snapshotLabel, snapshotContent, snapshotCards, netActivityContent, insiderRoleMix = [], insiderTopMovingSectors = [], insiderRecentNotableTrades = [], insiderMonthlyActivity = [], metrics, primary, secondary, sectorRows, sectorTitle, left, right, recent, note, lockedMessage, institutionalActivity = [] }: { flavor: DashboardFlavor; eyebrow: string; title: string; subtitle: string; filter: ReactNode; comparison: string; snapshotTitle: string; snapshotSeries: Array<{ label: string; value: number }>; snapshotLabel: string; snapshotContent?: ReactNode; snapshotCards?: SnapshotCardSpec[]; netActivityContent?: ReactNode; insiderRoleMix?: NonNullable<InsidersOverviewResponse["role_mix"]>; insiderTopMovingSectors?: NonNullable<InsidersOverviewResponse["top_moving_sectors"]>; insiderRecentNotableTrades?: ProfileActivityItem[]; insiderMonthlyActivity?: NonNullable<InsidersOverviewResponse["monthly_activity"]>; metrics: ProfileMetric[]; primary: TableSpec; secondary: TableSpec; sectorRows: ProfileSectorPeriod[]; sectorTitle: string; left: TableSpec; right: TableSpec; recent: ProfileActivityItem[]; note?: string; lockedMessage?: string | null; institutionalActivity?: InstitutionalActivityPeriod[] }) {
  ...
  const dashboardContent = <>
    ...
    <MetricCards metrics={metrics} comparison={comparison} flavor={flavor} insiderMonthlyActivity={insiderMonthlyActivity} institutionalActivity={institutionalActivity} />
...
function MetricCards({ metrics, comparison, flavor, insiderMonthlyActivity = [], institutionalActivity = [] }: { metrics: ProfileMetric[]; comparison: string; flavor?: DashboardFlavor; insiderMonthlyActivity?: NonNullable<InsidersOverviewResponse["monthly_activity"]>; institutionalActivity?: InstitutionalActivityPeriod[] }) {
...
function institutionalMetricSeries(label: string, rows: InstitutionalActivityPeriod[]) {
  const normalized = label.toLowerCase();
  return rows.slice(-8).map((row) => ({
    label: row.period,
    value: normalized.includes("tracked") ? Number(row.tracked_institutions ?? 0)
      : normalized.includes("portfolio") ? Number(row.portfolio_value ?? 0)
        : normalized.includes("increases") ? Number(row.position_increase_count ?? 0)
          : normalized.includes("decreases") ? Number(row.position_decrease_count ?? 0)
            : normalized.includes("net reported") ? Number(row.net_value_change ?? 0)
              : 0,
...
function TrendChart({ series }: { series: Array<{ label: string; value: number }> }) {
  ...
  const diamondPoints = (x: number, y: number) => `${x},${y - 1.15} ${x + .22},${y} ${x},${y + 1.15} ${x - .22},${y}`;
...
function SnapshotCard({ row, label, icon, valueKey, valueFormat, suffix = "", subLabelKey }: SnapshotCardSpec) { const header = <div className="flex items-center gap-3">{icon ? <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-emerald-300/25 bg-emerald-300/10 text-emerald-200"><SnapshotIcon name={icon} /></span> : null}<p className="text-[10px] uppercase tracking-[.14em] text-slate-500">{label}</p></div>; if (!row) return <div className="rounded-lg border border-slate-700/70 bg-slate-950/65 p-4">{header}<p className="mt-3 text-sm text-slate-400">No record available</p></div>; const title = stringAt(row, ["name", "symbol", "sector", "profile", "company"]) ?? "Live result"; const detail = valueKey ? asNumber(row[valueKey]) : valueAt(row, ["net_value", "net_buy_value", "increase_value", "portfolio_value", "value", "buy_value", "trades"]); const recentDate = stringAt(row, ["filing_date", "last_transaction", "last_activity", "recent_activity"]); const subLabel = subLabelKey ? stringAt(row, [subLabelKey]) : null; return <div className="rounded-lg border border-slate-700/70 bg-slate-950/65 p-4">{header}<p className="mt-3 truncate text-sm font-semibold text-white">{title}</p><p className="mt-2 text-sm font-semibold tabular-nums text-emerald-300">{detail !== null ? `${formatSnapshotValue(detail, valueFormat)}${suffix}` : recentDate ? formatDate(recentDate) : "-"}</p>{subLabel ? <p className="mt-1 text-xs text-slate-400">{subLabel}</p> : null}</div>; }
function SnapshotIcon({ name }: { name: SnapshotIconName }) { const common = { fill: "none", stroke: "currentColor", strokeLinecap: "round" as const, strokeLinejoin: "round" as const, strokeWidth: 1.8 }; const paths: Record<SnapshotIconName, ReactNode> = { buyer: <><circle cx="8" cy="7.5" r="2.5" {...common} /><path d="M3.5 16.5c.8-3 2.3-4.5 4.5-4.5s3.7 1.5 4.5 4.5" {...common} /><path d="M15 5v6m-3-3h6" {...common} /></>, ticker: <><path d="M4 15.5h12" {...common} /><path d="M5 12l3-3 2.5 2.5L15 6" {...common} /><path d="M13 6h2v2" {...common} /></>, cluster: <><circle cx="6" cy="7" r="2.2" {...common} /><circle cx="14" cy="7" r="2.2" {...common} /><circle cx="10" cy="13" r="2.2" {...common} /><path d="M7.8 8.4 9 10.8m3.2-2.4L11 10.8" {...common} /></>, sector: <><path d="M10 3.5v13" {...common} /><path d="M16.5 10A6.5 6.5 0 1 1 10 3.5" {...common} /><path d="M10 10h6.5" {...common} /></> }; return <svg viewBox="0 0 20 20" aria-hidden="true" className="h-4 w-4">{paths[name]}</svg>; }
function formatSnapshotValue(value: number, format?: "currency" | "number" | "percent") { if (format === "number") return formatNumber(value); if (format === "percent") return `${value.toFixed(1)}%`; return formatMoney(value); }
...
function DirectoryLeaders({ title, href, rows, locked = false, message }: { title: string; href: string; rows: Array<{ label: string; value?: number | null; value_format?: string; href?: string | null }>; locked?: boolean; message?: string | null }) { const visibleRows = rows.slice(0, 5); const max = Math.max(...visibleRows.map((row) => Math.abs(row.value ?? 0)), 1); const emptyMessage = locked ? message ?? "Upgrade to Pro to unlock this ranking." : "No current ranking data is available."; return <div className="min-w-0 border-l border-white/10 pl-3 first:border-l-0 first:pl-0"><h3 className="truncate text-[11px] font-semibold text-white">{title}</h3>{visibleRows.length ? <div className="mt-2 space-y-1.5">{visibleRows.map((row, index) => { const label = row.href ? <Link href={row.href} prefetch={false} className="truncate text-slate-300 hover:text-emerald-100">{row.label}</Link> : <span className="truncate text-slate-300">{row.label}</span>; const width = Math.max(3, (Math.abs(row.value ?? 0) / max) * 100); return <div key={`${row.label}-${index}`} className="grid grid-cols-[0.9rem_minmax(0,1fr)_minmax(2rem,3.2rem)_4.2rem] items-center gap-1.5 text-[11px]"><span className="text-slate-500">{index + 1}</span>{label}<span className="h-px overflow-hidden rounded bg-slate-800"><i className="block h-full rounded bg-emerald-300" style={{ width: `${width}%` }} /></span><span className="text-right font-semibold tabular-nums text-white">{row.value_format === "currency" ? formatMoney(row.value) : formatNumber(row.value)}</span></div>; })}</div> : <p className="mt-2 min-h-10 text-[11px] leading-5 text-slate-400">{emptyMessage}</p>}<Link href={locked ? "/pricing" : href} className="mt-3 inline-flex text-[11px] font-semibold text-emerald-200">{locked ? "View Pro access" : `View ${title}`} -&gt;</Link></div>; }
...
function InsiderRoleMixDonut({ rows }: { rows: NonNullable<InsidersOverviewResponse["role_mix"]> }) { const visible = rows.filter((row) => row.value > 0).slice(0, 5); if (!visible.length) return <p className="py-8 text-sm text-slate-400">No insider role data is available.</p>; const colors = ["#f6b91a", "#42d3a7", "#5b5cf6", "#3b82f6", "#94a3b8"]; const total = visible.reduce((sum, row) => sum + row.value, 0); return <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><Donut values={visible.map((row) => row.value)} colors={colors} label="Total trades" value={formatNumber(total)} /><div className="min-w-0 flex-1 space-y-2.5">{visible.map((row, index) => <LegendRow key={row.label} label={row.label} color={colors[index]} value={`${row.percent.toFixed(1)}%`} />)}</div></div>; }
function InsiderTopMovingSectors({ rows }: { rows: NonNullable<InsidersOverviewResponse["top_moving_sectors"]> }) { const visible = rows.slice(0, 6); if (!visible.length) return <p className="py-8 text-sm text-slate-400">No comparable sector movement is available.</p>; return <div className="overflow-hidden"><table className="w-full table-fixed text-left text-xs"><thead className="border-b border-white/10 text-[10px] uppercase tracking-[.13em] text-slate-500"><tr><th className="w-[31%] px-2 py-2">Sector</th><th className="w-[21%] px-2 py-2 text-right">Net activity (TTM)</th><th className="w-[18%] px-2 py-2 text-right">Vs prior TTM</th><th className="w-[30%] px-2 py-2">Trend</th></tr></thead><tbody className="divide-y divide-white/5">{visible.map((row) => { const positive = typeof row.change_pct !== "number" || row.change_pct >= 0; return <tr key={row.sector}><td className="truncate px-2 py-2.5 font-semibold text-slate-300">{row.sector}</td><td className={row.current_value >= 0 ? "px-2 py-2.5 text-right font-semibold tabular-nums text-emerald-300" : "px-2 py-2.5 text-right font-semibold tabular-nums text-rose-300"}>{formatSignedMoney(row.current_value)}</td><td className={positive ? "px-2 py-2.5 text-right font-semibold tabular-nums text-emerald-300" : "px-2 py-2.5 text-right font-semibold tabular-nums text-rose-300"}>{typeof row.change_pct === "number" ? `${row.change_pct >= 0 ? "+" : ""}${row.change_pct.toFixed(1)}%` : "-"}</td><td className="px-2 py-2.5"><CompactSparkline points={row.trend ?? []} positive={positive} /></td></tr>; })}</tbody></table></div>; }
function InsiderNotableTrades({ items }: { items: ProfileActivityItem[] }) { return <Panel title="Recent notable trades">{items.length ? <div className="overflow-x-auto"><table className="min-w-[34rem] w-full text-left text-xs"><thead className="border-b border-white/10 text-[10px] uppercase tracking-[.13em] text-slate-500"><tr>{["Insider", "Ticker", "Action", "Value", "Date"].map((label) => <th key={label} className="px-2 py-2">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/5">{items.slice(0, 7).map((item) => { const action = congressTradeAction(item.activity); return <tr key={String(item.id)}><td className="max-w-44 truncate px-2 py-2.5 font-semibold text-emerald-200">{item.profile}</td><td className="px-2 py-2.5 text-slate-300">{item.symbol ?? "-"}</td><td className={`px-2 py-2.5 font-semibold ${action.className}`}>{action.label}</td><td className="px-2 py-2.5 font-semibold tabular-nums text-white">{formatMoney(item.value)}</td><td className="px-2 py-2.5 text-slate-400">{item.time ? formatDate(item.time) : "-"}</td></tr>; })}</tbody></table></div> : <p className="py-8 text-sm text-slate-400">No recent trades available.</p>}</Panel>; }
function InsiderActivityOverTime({ rows }: { rows: NonNullable<InsidersOverviewResponse["monthly_activity"]> }) { const points = rows.slice(-12).map((row) => ({ label: row.period, buy: Number(row.buy_value ?? 0), sell: Number(row.sell_value ?? 0), trades: Number(row.trades ?? 0) })); if (!points.length) return <p className="flex h-56 items-center justify-center text-sm text-slate-400">No monthly insider activity is available.</p>; const valueLimit = roundAxisMax(Math.max(...points.flatMap((point) => [point.buy, point.sell]), 1)); const valueTicks = [valueLimit, valueLimit / 2, 0, -valueLimit / 2, -valueLimit]; const tradeLimit = roundAxisMax(Math.max(...points.map((point) => point.trades), 1)); const tradeTicks = [tradeLimit, tradeLimit * .75, tradeLimit * .5, tradeLimit * .25, 0]; const plotTop = 8; const plotBottom = 92; const zeroY = 50; const xFor = (index: number) => 5 + (index / Math.max(points.length - 1, 1)) * 90; const yValue = (value: number) => zeroY - (Math.max(-valueLimit, Math.min(valueLimit, value)) / valueLimit) * ((plotBottom - plotTop) / 2); const yTrades = (value: number) => plotBottom - (Math.max(value, 0) / tradeLimit) * (plotBottom - plotTop); const barWidth = Math.min(3.2, 22 / points.length); const tradeLine = points.map((point, index) => `${xFor(index)},${yTrades(point.trades)}`).join(" "); const minChartWidth = Math.max(42, points.length * 4.5); return <div className="min-w-0"><div className="mb-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-slate-400"><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm bg-emerald-400" />Buy Value (USD)</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm bg-rose-400" />Sell Value (USD)</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-blue-400" />Total Trades</span></div><div className="grid grid-cols-[3.8rem_minmax(0,1fr)_3.5rem] gap-3"><div><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">Value</p><div className="flex h-56 flex-col justify-between text-right text-[10px] font-semibold tabular-nums text-slate-500">{valueTicks.map((tick) => <span key={tick}>{formatAxisValue(tick)}</span>)}</div></div><div className="min-w-0 overflow-x-auto pb-1"><div style={{ minWidth: `${minChartWidth}rem` }}><div className="relative h-56 overflow-hidden"><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full overflow-hidden border-b border-white/10" aria-label="Monthly insider buy value, sell value, and total trades">{valueTicks.map((tick) => <line key={tick} x1="0" x2="100" y1={yValue(tick)} y2={yValue(tick)} stroke="rgba(148,163,184,.16)" vectorEffect="non-scaling-stroke" />)}<line x1="0" x2="100" y1={zeroY} y2={zeroY} stroke="rgba(148,163,184,.3)" vectorEffect="non-scaling-stroke" />{points.map((point, index) => { const x = xFor(index); const buyY = yValue(point.buy); const sellY = yValue(-point.sell); return <g key={point.label}><rect x={x - barWidth - .35} y={buyY} width={barWidth} height={zeroY - buyY} rx=".45" fill="#42d3a7" opacity=".78" /><rect x={x + .35} y={zeroY} width={barWidth} height={sellY - zeroY} rx=".45" fill="#fb7185" opacity=".78" /></g>; })}<polyline points={tradeLine} fill="none" stroke="#60a5fa" strokeWidth="1.8" vectorEffect="non-scaling-stroke" /></svg>{points.map((point, index) => <span key={`${point.label}-activity-trade-dot`} className="absolute h-2 w-2 rounded-full border border-blue-100 bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,.75)]" style={{ left: `calc(${xFor(index)}% - 4px)`, top: `calc(${yTrades(point.trades)}% - 4px)` }} />)}</div><div className="mt-2 grid gap-1 text-[10px] text-slate-500" style={{ gridTemplateColumns: `repeat(${points.length}, minmax(0, 1fr))` }}>{points.map((point) => <span key={point.label} className="truncate text-center">{point.label}</span>)}</div></div></div><div><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">Total trades</p><div className="flex h-56 flex-col justify-between text-left text-[10px] font-semibold tabular-nums text-slate-500">{tradeTicks.map((tick) => <span key={tick}>{formatCompactNumber(tick)}</span>)}</div></div></div></div>; }
...
function InstitutionalActivityOverTime({ rows }: { rows: InstitutionalActivityPeriod[] }) { const periods = rows.slice(-8); if (!periods.length) return <p className="flex h-48 items-center justify-center text-sm text-slate-400">No quarterly institutional activity is available.</p>; const valueLimit = 2_000_000_000_000; const valueTicks = [valueLimit, valueLimit / 2, 0, -valueLimit / 2, -valueLimit]; const totalMax = Math.max(...periods.map((row) => row.total_positions), 1); const totalTop = roundAxisMax(totalMax); const xFor = (index: number) => 8 + (index / Math.max(periods.length - 1, 1)) * 84; const yValue = (value: number) => 50 - (Math.max(-valueLimit, Math.min(valueLimit, value)) / valueLimit) * 40; const yTotal = (value: number) => 90 - (Math.max(0, value) / totalTop) * 80; const barWidth = Math.min(3.8, 22 / periods.length); const totalLine = periods.map((row, index) => `${xFor(index)},${yTotal(row.total_positions)}`).join(" "); return <div className="min-w-0"><div className="mb-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-slate-400"><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm bg-emerald-400" />Position increases</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm bg-rose-400" />Position decreases</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-blue-400" />Total positions</span></div><div className="grid grid-cols-[3.6rem_minmax(0,1fr)_4.25rem] gap-3"><div><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">Value change</p><div className="flex h-48 flex-col justify-between text-right text-[10px] font-semibold tabular-nums text-slate-500">{valueTicks.map((tick) => <span key={tick}>{formatAxisValue(tick)}</span>)}</div></div><div className="min-w-0"><div className="relative h-48 overflow-hidden"><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full overflow-hidden">{[10, 30, 50, 70, 90].map((y) => <line key={y} x1="0" x2="100" y1={y} y2={y} stroke="rgba(148,163,184,.16)" vectorEffect="non-scaling-stroke" />)}{periods.map((row, index) => { const x = xFor(index); const incY = yValue(row.position_increase_value); const decY = yValue(row.position_decrease_value); return <g key={row.period}><rect x={x - barWidth - 0.4} y={incY} width={barWidth} height={50 - incY} rx=".5" fill="#42d3a7" opacity=".78" /><rect x={x + 0.4} y={50} width={barWidth} height={decY - 50} rx=".5" fill="#fb7185" opacity=".78" /></g>; })}<line x1="0" x2="100" y1="50" y2="50" stroke="rgba(148,163,184,.28)" vectorEffect="non-scaling-stroke" /><polyline points={totalLine} fill="none" stroke="#60a5fa" strokeWidth="1.6" vectorEffect="non-scaling-stroke" /></svg>{periods.map((row, index) => <span key={`${row.period}-dot`} className="absolute h-2 w-2 rounded-full border border-blue-100 bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,.75)]" style={{ left: `calc(${xFor(index)}% - 4px)`, top: `calc(${yTotal(row.total_positions)}% - 4px)` }} />)}</div><div className="mt-2 grid gap-1 text-[10px] text-slate-500" style={{ gridTemplateColumns: `repeat(${periods.length}, minmax(0, 1fr))` }}>{periods.map((row) => <span key={row.period} className="truncate text-center">{row.period}</span>)}</div></div><div><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">Total positions</p><div className="flex h-48 flex-col justify-between text-left text-[10px] font-semibold tabular-nums text-slate-500"><span>{formatNumber(totalTop)}</span><span>{formatNumber(totalTop / 2)}</span><span>0</span></div></div></div></div>; }
...
function formatSignedMoney(value?: number | null) { if (typeof value !== "number" || !Number.isFinite(value)) return "-"; if (value === 0) return "$0"; const formatted = formatMoney(Math.abs(value)); return value > 0 ? `+${formatted}` : `-${formatted}`; }
...
function roundAxisMax(value: number) { if (!Number.isFinite(value) || value <= 0) return 1; const magnitude = 10 ** Math.floor(Math.log10(value)); return Math.ceil(value / magnitude) * magnitude; }
