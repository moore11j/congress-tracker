    ...
    <main className="relative min-w-0 space-y-3 overflow-hidden pb-3">
      ...
      <section className="relative z-10 grid items-stretch gap-3 xl:grid-cols-[1.45fr_.8fr_1.1fr]">
        ...
        <Panel title="Where the activity is">
          <Donut values={counts} labels={categories} colors={categories.map((category) => PROFILE_COLORS[category])} label="Records" value={formatNumber(total)} />
...
function ChamberMix({ rows }: { rows: Array<{ label: string; value: number }> }) { return <Panel title="Chamber mix" subtitle="Reported trade value">{rows.length ? <div className="flex items-center gap-3"><Donut values={rows.map((row) => row.value)} labels={rows.map((row) => row.label)} colors={["#3b82f6", "#a855f7", "#94a3b8"]} label="Value" value={formatMoney(rows.reduce((sum, row) => sum + row.value, 0))} /><div className="flex-1 space-y-2">{rows.map((row, index) => <LegendRow key={row.label} label={row.label} color={["#3b82f6", "#a855f7", "#94a3b8"][index]} value={formatMoney(row.value)} />)}</div></div> : <p className="py-8 text-sm text-slate-400">No chamber data available.</p>}</Panel>; }
