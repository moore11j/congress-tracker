  ...
  return (
    <main className="relative min-w-0 space-y-3 overflow-hidden pb-3">
      ...
      <section className="relative z-10 grid gap-3 xl:grid-cols-[1.25fr_.8fr_1.1fr]">
        <Panel title="Latest activity by profile type" subtitle="Monthly activity over the past two years" action="Live database data">
...
function MetricDelta({ metric }: { metric?: ProfileMetric }) { return typeof metric?.change_pct === "number" ? <p className={metric.change_pct >= 0 ? "mt-3 text-xs font-semibold text-emerald-300" : "mt-3 text-xs font-semibold text-rose-300"}>{metric.change_pct >= 0 ? "+" : ""}{metric.change_pct.toFixed(1)}% vs prior 12 months</p> : <p className="mt-3 text-xs text-slate-500">Prior period unavailable</p>; }
function CongressTrend({ points }: { points: Array<{ label: string; value: number }> }) { const max = Math.max(...points.map((point) => point.value), 1); const line = points.map((point, index) => `${(index / Math.max(points.length - 1, 1)) * 100},${96 - (point.value / max) * 82}`).join(" "); return <div><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-36 w-full overflow-visible">{[25, 50, 75].map((y) => <line key={y} x1="0" x2="100" y1={y} y2={y} stroke="rgba(148,163,184,.15)" vectorEffect="non-scaling-stroke" />)}<polyline points={line} fill="none" stroke="#55e3b0" strokeWidth="1.7" vectorEffect="non-scaling-stroke" /></svg><div className="mt-2 flex justify-between text-[10px] text-slate-500"><span>{points[0]?.label}</span><span>{points.at(-1)?.label}</span></div></div>; }
function CongressSnapshot({ icon, label, row, valueKey, suffix = "" }: { icon: string; label: string; row?: Row; valueKey: string; suffix?: string }) { const title = row ? stringAt(row, ["name", "symbol", "sector", "company"]) ?? "No reported record" : "No reported record"; const value = row?.[valueKey]; const display = typeof value === "number" ? `${valueKey === "trades" ? formatNumber(value) : formatMoney(value)}${suffix}` : "-"; return <div className="rounded-lg border border-slate-700/70 bg-slate-950/65 p-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-300/25 bg-emerald-300/10 text-base text-emerald-200">{icon}</span><p className="text-[10px] uppercase tracking-[.14em] text-slate-500">{label}</p></div><p className="mt-3 truncate text-sm font-semibold text-white">{title}</p><p className="mt-1 text-xs font-semibold text-emerald-300">{display}</p></div>; }
function CongressMetric({ metric, monthly }: { metric: ProfileMetric; monthly: CongressPoint[] }) { const key = metric.label === "Total Trades" ? "trades" : metric.label === "Total Buy Value" ? "buy_value" : metric.label === "Total Sell Value" ? "sell_value" : metric.label === "Active Members" ? "active_members" : "average_trade_size"; return <div className="rounded-lg border border-slate-700/70 bg-slate-950/65 p-4"><p className="truncate text-[10px] uppercase tracking-[.14em] text-slate-500">{metric.label}</p><p className="mt-3 text-2xl font-semibold text-white">{formatMetric(metric)}</p><MetricDelta metric={metric} /><div className="mt-3"><CongressTrend points={monthly.map((row) => ({ label: row.period, value: Number(row[key] ?? 0) }))} /></div></div>; }
...
function CongressMovers({ rows }: { rows: NonNullable<CongressOverviewResponse["top_moving_sectors"]> }) { return <Panel title="Top moving sectors" subtitle="Vs prior 12 months"><div className="space-y-3">{rows.map((row) => <div key={row.sector} className="grid grid-cols-[1fr_4rem_4rem] items-center gap-2 text-xs"><span className="truncate text-slate-300">{row.sector}</span><span className={typeof row.change_pct === "number" && row.change_pct >= 0 ? "text-emerald-300" : "text-rose-300"}>{typeof row.change_pct === "number" ? `${row.change_pct >= 0 ? "+" : ""}${row.change_pct.toFixed(1)}%` : "-"}</span><CongressTrend points={row.trend.slice(-6)} /></div>)}</div></Panel>; }
function NotableTrades({ items }: { items: ProfileActivityItem[] }) { return <Panel title="Recent notable trades">{items.length ? <div className="overflow-x-auto"><table className="min-w-[31rem] w-full text-left text-xs"><thead className="border-b border-white/10 text-[10px] uppercase tracking-[.13em] text-slate-500"><tr>{["Member", "Ticker", "Action", "Value", "Date"].map((label) => <th key={label} className="px-2 py-2">{label}</th>)}</tr></thead><tbody>{items.slice(0, 7).map((item) => <tr key={String(item.id)} className="border-b border-white/5"><td className="px-2 py-2.5 font-semibold text-emerald-200">{item.profile}</td><td className="px-2 py-2.5 text-slate-300">{item.symbol ?? "-"}</td><td className="px-2 py-2.5 text-slate-300">{item.activity ?? "-"}</td><td className="px-2 py-2.5 text-white">{formatMoney(item.value)}</td><td className="px-2 py-2.5 text-slate-400">{item.time ? formatDate(item.time) : "-"}</td></tr>)}</tbody></table></div> : <p className="py-8 text-sm text-slate-400">No recent trades available.</p>}</Panel>; }
