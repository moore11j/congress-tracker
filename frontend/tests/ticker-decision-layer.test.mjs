...
test("macro positioning presents coherent regime counts in a compact responsive factor layout", () => {
  assert.match(card, /How the current market environment affects \{symbol\}/);
  assert.match(card, /Bullish factor/);
  assert.match(card, /Bearish factor/);
  assert.match(card, /Neutral factor/);
