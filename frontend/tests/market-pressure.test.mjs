...
test("Compare appears between Backtesting and Maps in the shared top navigation", () => {
  ...
  const compareIndex = nav.indexOf('{ href: "/compare", label: "Compare" }');
  ...
  assert.match(nav, /basePath === "\/compare"/);
...
test("Market Pressure share and analytics events are wired through first-party helpers", () => {
  ...
  assert.match(client, /market_pressure_colour_mode_changed/);
...
test("Market Pressure Phase 3 visual semantics are source-driven", () => {
  ...
  assert.match(client, /function confirmationFillClass/);
  ...
  assert.match(client, /return "border border-slate-950"/);
  assert.doesNotMatch(client, /border-dashed border-amber-200/);
  assert.doesNotMatch(client, /stroke-dasharray/);
...
test("Market Pressure map uses a sector-nested treemap layout", () => {
  ...
  assert.match(client, /const labelRect = displayRect \?\? rect/);
  assert.match(client, /displayRect=\{displayRect\}/);
