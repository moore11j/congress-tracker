...
const TICKER_CONTEXT_BUNDLE_CACHE_VERSION = 11;
...
export type CongressOverviewResponse = {
  ...
  sector_activity?: Array<{ sector: string; current_value: number; previous_value: number; current_activity_value: number; previous_activity_value: number; change_pct: number | null; buy_value: number; sell_value: number; trades: number; trade_percent: number; trend: Array<{ label: string; value: number }> }>;
  chamber_mix?: Array<{ label: string; value: number }>;
  top_moving_sectors?: Array<{ sector: string; current_value: number; previous_value: number; current_activity_value: number; previous_activity_value: number; change_pct: number | null; buy_value: number; sell_value: number; trades: number; trade_percent: number; trend: Array<{ label: string; value: number }> }>;
...
export type TickerAnalystConsensusAccess = {
  detailLevel?: "current_summary" | "full_detail" | string;
  detailsLocked?: boolean;
  requiredPlanForDetails?: "premium" | string | null;
};
...
export type TickerAnalystConsensusSnapshot = {
  ...
  weightedRatingValue?: number | null;
  priceTargetRange?: {
    high?: number | null;
    low?: number | null;
    median?: number | null;
...
export type TickerAnalystConsensusChange = {
  comparisonDate?: string | null;
  daysBetween?: number | null;
  weightedSentimentChange?: number | null;
  medianTargetChange?: number | null;
...
export type TickerAnalystConsensusResponse = {
  ...
  gradeEventStats?: {
    ...
    days90?: TickerAnalystConsensusGradeWindow;
    mostRecentEvent?: {
      action?: string | null;
  ...
  interpretation?: {
    currentAnalystDirection?: string | null;
    trendDirection?: string | null;
    combinedLabel?: string | null;
    coverageLevel?: string | null;
...
export type AdminResearchBriefValidation = {
...
export type AdminResearchBriefDraft = {
  id: string;
  status: "generating" | "draft" | "ready_for_review" | "published" | "unpublished" | "failed" | string;
  created_by?: number | null;
...
export async function validateAdminResearchBriefTicker(symbol: string): Promise<{ symbol: string; identity: Record<string, unknown> }> {
...
export async function publishAdminResearchBriefDraft(draftId: string): Promise<AdminResearchBriefDraft> {
  return fetchJson<AdminResearchBriefDraft>(buildApiUrl(`/api/admin/research-briefs/drafts/${encodeURIComponent(draftId)}/publish`), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function unpublishAdminResearchBriefDraft(draftId: string): Promise<AdminResearchBriefDraft> {
  return fetchJson<AdminResearchBriefDraft>(buildApiUrl(`/api/admin/research-briefs/drafts/${encodeURIComponent(draftId)}/unpublish`), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ confirm: true }),
    source: "AdminResearchBriefs",
...
export async function deleteAdminResearchBriefDraft(draftId: string): Promise<{ ok: boolean; deleted: string }> {
  return fetchJson<{ ok: boolean; deleted: string }>(buildApiUrl(`/api/admin/research-briefs/drafts/${encodeURIComponent(draftId)}`), {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
...
export async function getTickerAnalystConsensus(
  symbol: string,
  params?: { authToken?: string | null; signal?: AbortSignal; source?: string },
): Promise<TickerAnalystConsensusResponse> {
  const url = buildApiUrl(`/api/tickers/${tickerPathSymbol(symbol)}/consensus`);
  return clientCachedJson<TickerAnalystConsensusResponse>(
    `ticker-analyst-consensus:${url}:${params?.authToken ? "auth" : "session"}`,
    params?.signal,
    (signal) => fetchJson<TickerAnalystConsensusResponse>(url, {
      headers: authHeaders(params?.authToken ?? undefined),
      cache: "no-store",
      next: { revalidate: 0 },
      signal,
