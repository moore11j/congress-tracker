...
import { recordGoogleAnalyticsEvent } from "./googleAnalytics";
...
export function trackEvent(name: FunnelEvent, properties: AnalyticsProperties = {}): boolean {
  try {
    ...
    for (const send of [
      () => recordProductEvent({ event_name: name, path: route, properties: clean }),
