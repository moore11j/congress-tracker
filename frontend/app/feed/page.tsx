...
function partyAbbrev(p?: string | null) {
  ...
  return p.trim().slice(0, 6).toUpperCase();
}

function partyTone(p?: string | null): BadgeTone {
  const v = (p ?? "").trim().toLowerCase();
  if (!v) return "neutral";
...
function pillStyle(tone: BadgeTone): React.CSSProperties {
  ...
  if (tone === "dem") return { ...base, background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.25)" };
  if (tone === "rep") return { ...base, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.25)" };
  if (tone === "ind") return { ...base, background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.22)" };

  if (tone === "pos") return { ...base, background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.22)" };
  if (tone === "neg") return { ...base, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.22)" };

  // house/senate/neutral fall back to base
  return base;
}
    ...
    >
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "28px 20px 40px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ fontSize: 12, opacity: 0.7 }}>
              Updated: <span style={{ fontWeight: 900, opacity: 0.95 }}>{formatLastUpdated(meta.last_updated_utc)}</span>
            </div>
        ...
        <div style={{ marginBottom: 14 }}>
          <h1 style={{ fontSize: 40, lineHeight: 1.05, margin: 0, fontWeight: 950, letterSpacing: -0.8 }}>
            Congress Trades
          </h1>
          <div style={{ marginTop: 10, fontSize: 14, opacity: 0.75 }}>
            Filter by ticker, member, chamber, type, and minimum amount.
          </div>
        </div>
        ...
        >
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "1.2fr 1.4fr 0.9fr 0.9fr",
              gap: 12,
              alignItems: "end",
            }}
          >
        ...
        <div style={{ display: "grid", gap: 14 }}>
          {data.items.length === 0 ? (
            ...
            >
              ...
              <div style={{ marginTop: 6, fontSize: 13 }}>Try clearing filters or lowering min amount.</div>
            </div>
          ) : (
            data.items.map((it) => {
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "flex-start" }}>
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
                        ...
                        <div style={{ fontSize: 14, opacity: 0.92, minWidth: 0 }}>
                          <span style={{ fontWeight: 850 }}>{it.security.name}</span>
                      ...
                      <div style={{ marginTop: 10, fontSize: 13, opacity: 0.75 }}>
                        Trade: <span style={{ fontWeight: 900 }}>{it.trade_date ?? "—"}</span>
                        <span style={{ opacity: 0.45 }}> · </span>
                        Report: <span style={{ fontWeight: 900 }}>{it.report_date ?? "—"}</span>
                      </div>
                    </div>
