...
export default async function MemberPage({ params, searchParams }: Props) {
  ...
  const recentFeedItems = memberTradeItems.map((trade) => {
    ...
    return {
      ...
      estimated_trade_value: trade.estimated_trade_value ?? null,
      estimated_price: trade.estimated_price ?? null,
      estimated_shares: trade.estimated_shares ?? null,
      current_price: trade.current_price ?? null,
      pnl_pct: trade.pnl_pct ?? null,
      pnl_source: (trade.pnl_source as FeedItem["pnl_source"]) ?? null,
      outcome_status: trade.outcome_status ?? null,
      outcome_skip_reason: trade.outcome_skip_reason ?? null,
      outcome_methodology: trade.outcome_methodology ?? null,
      outcome_error: trade.outcome_error ?? null,
      price_basis: trade.price_basis ?? null,
