...
- `prod200`: production-only watched profile that ramps to 100 VUs over 4m, 150 VUs over 5m, 200 VUs over 5m, holds 200 VUs for 2m, then ramps down over 4m. Requires explicit separate approval for the exact run; do not substitute `large`.
- `prod300`: production-only watched profile that ramps to 150 VUs over 4m, 250 VUs over 5m, 300 VUs over 4m, holds 300 VUs for 2m, then ramps down over 4m. Requires explicit separate approval for the exact run.
- `prod400`: production-only watched profile that ramps to 200 VUs over 4m, 300 VUs over 5m, 400 VUs over 5m, holds 400 VUs for 2m, then ramps down over 4m. Requires explicit separate approval for the exact run.
...
`medium` must not be used as a substitute for a production 75-VU run because it ramps to 100 VUs. `large` must not be used as a substitute for a production 200/300/400-VU run because it ramps to 250 VUs with a different shape. Production runs at 100+ VUs require separate explicit approval for the exact profile and command.
